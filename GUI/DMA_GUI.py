...
#! /usr/bin/env python     a shebang tells the program loader that this script is executed with python
# -*- coding: utf-8 -*-     so you don't have to type the word python before running the .py
...
def RUN_ME(b1):
    ...
    subprocess.run(["vsim", "-c", "-do", "run 3000",  "work.processor_tb", "-c", "-do", "exit"])
    MainFrame.step_count = 2
...
# setting up the simulation environment for the first time
    ...
    clock = 10          # to be adjusted
    
    # if( MainFrame.time_of_interrupt)
        # subprocess.run(["vsim", "-c", "-do", "run {} ns".format(MainFrame.step_count*clock),  "work.processor_tb", "-c", "-do", "exit"])
    ...
    # file.write("runned: {}".format(MainFrame.step_count*clock))
    # file.close()
    Import_Processor_Status()
    Import_DMA_Status()
    Import_Memory_Status()
...
def IO1_INT(b1):
    ...
    MainFrame.time_of_interrupt = MainFrame.step_count
   # subprocess.run(["vsim", "-c", "-do", "run {} ns".format(MainFrame.step_count*clock),  "work.processor_tb", "-c", "-do", "exit"])
    file.write("1")
...
def save_binary(): #####################################
    cwd = os.getcwd()
    file = open('{}/instructionfile.txt'.format(cwd), 'w')
    file.write(w.binary.get("1.0","end-1c"))


def save_io1():
    cwd = os.getcwd() #####################################
    file = open('{}/buffermemory1.mem'.format(cwd), 'w')
    file.write(w.init_io1.get("1.0","end-1c"))

 
def save_io2():
    cwd = os.getcwd() #####################################
    file = open('{}/buffermemory2.mem'.format(cwd), 'w')
